...
class Padaria(models.Model):
    ...
    horario_inicio = models.TimeField(
        verbose_name="Horário de Abertura", null=True, blank=True, help_text="Horário de abertura da padaria")
    horario_fim = models.TimeField(
        verbose_name="Horário de Fechamento", null=True, blank=True, help_text="Horário de fechamento da padaria")

    @property
    def image_url(self):
        """Retorna a URL completa da imagem"""
        if self.imagem and hasattr(self.imagem, 'url'):
            return f'http://localhost:8000{self.imagem.url}'
        return None
